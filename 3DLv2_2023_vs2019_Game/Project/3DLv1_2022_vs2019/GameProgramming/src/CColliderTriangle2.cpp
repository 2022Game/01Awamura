...
CColliderTriangle2::CColliderTriangle2(CCharacter3* parent, CMatrix* matrix
	, const CVector& v0, const CVector& v1, const CVector& v2)
{
	Set(parent, matrix, v0, v1, v2);
}

void CColliderTriangle2::Set(CCharacter3* parent, CMatrix* matrix
	, const CVector& v0, const CVector& v1, const CVector& v2) {
	mType = ETRIANGLE3;//三角コライダ
	mpParent = parent;//親設定
	if (matrix)
...
void CColliderTriangle2::Render()
{
	//行列退避
	glPushMatrix();
	//行列適用
...
//優先度の変更
void CColliderTriangle2::ChangePriority()
{
	//ｍV[0]とｍV[2]の中心を求める
